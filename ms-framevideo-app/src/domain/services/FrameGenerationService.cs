...
{
    public class FrameGenerationService : IFrameGenerationServicePort
    {
        /// <summary>
        ...
        public List<Frame> GenerateFrames(Chunk chunk, string localChunkFilePath)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(localChunkFilePath))
                {
                    throw new ArgumentException("O caminho do arquivo do chunk não pode ser nulo ou vazio.", nameof(localChunkFilePath));
                }
